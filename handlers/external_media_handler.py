...
def handle_agent_joined_event(channel_id, port, role, uniqueid):
    """Triggered when an agent joins a Vicidial room/channel."""
    try:
        # Call setup_single_channel_stream from ari_service
        ...
        logger.info(f"Successfully hooked audio stream for {channel_id}")
    except Exception as e:
        logger.error(f"Failed to setup stream for {channel_id}: {e}")
...
def process_bridge_start(event, ami_client):
    ...
    try:
        ...
        handle_agent_joined_event(channel, target_port, role, uniqueid)
